...
def make_control_panel(m):
    def create_expanded_button(description):
        return widgets.Button(description=description, button_style='warning', layout=widgets.Layout(height='auto', width='auto'))
    ...
    bppt1x.on_click(functools.partial(button_move, dx=.1, dy=0))
    control_panel[3, 4] = bppt1x

    bmpt1x = create_expanded_button('-.1mm')
    bmpt1x.on_click(functools.partial(button_move, dx=-.1, dy=0))
