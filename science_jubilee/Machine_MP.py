...
class Machine:
    ...
    def gcode(self, cmd: str = "", response_wait: float = 10):
        ...
        old_reply_count = requests.get(f"http://192.168.1.2/rr_model?key=seqs").json()[
            "result"
        ]["reply"]
        ...
        tic = time.time()
        while True:
            new_reply_count = requests.get(
                f"http://192.168.1.2/rr_model?key=seqs"
            ).json()["result"]["reply"]
    ...
    def load_tool(self, tool: Tool = None):
        ...
        if idx in self.tools:
            raise MachineConfigurationError(
                "Error: tool name already used by a different tool"
            )
        else:
            pass
        ...
        if name in set(self.tools.values()):
            raise MachineConfigurationError(
                "Error: tool index already set for a different tool"
            )
        else:
            pass
        self.tools[idx] = name
