...
class LabAutomationPlate(Plate):
    ...
    def __init__(self, machine, name, config):
        ...
        if not os.path.isfile(config_path):
            raise PlateConfigurationError(
                f"Error: config file at {config_path} does not exist"
            )
    ...
    def load_labware(self, slot_index, labware_name):
        ...
        labware_config_path = os.path.join(
            self.get_root_dir(), f"config/labware/{labware_name}.json"
        )

        if not os.path.isfile(labware_config_path):
            raise PlateConfigurationError(
                f"Error: Labware config file {labware_config_path} does not exist"
            )
        ...
        max_row_letter = chr(
            ord("@") + row_count
        )  # Labware rows are identified with letters
        ...
        b = [
            sum(x)
            for x in zip(
                slot["calibration_positions"][f"A{column_count}"], slot["origin"]
            )
        ]
        ...
        c = [
            sum(x)
            for x in zip(
                slot["calibration_positions"][f"{max_row_letter}{column_count}"],
                slot["origin"],
            )
        ]
    ...
    def get_well_position(self, slot_index, well_id):
        ...
        if (
            row <= 0
            or column <= 0
            or row > slot["row_count"]
            or column > slot["column_count"]
        ):
            raise PlateStateError(
                f"Error: Well ID ({row},{column}) is out of range for this labware."
            )
